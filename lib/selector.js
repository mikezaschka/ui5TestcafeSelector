...
exports.default = (0, _testcafe.Selector)(id => {
    /****************************************/
    //GLOBALS
    /****************************************/
    ...
    var fnGetBindingInformation = function fnGetBindingInformation(oItem, sBinding) {
    ...
    var _getParentWithDom = function _getParentWithDom(oItem, iCounter) {
    ...
    var _getUi5LocalId = function _getUi5LocalId(oItem) {
        var sId = oItem.getId();
    ...
    var _getItemForItem = function _getItemForItem(oItem) {
    ...
    var _getAllLabels = function _getAllLabels() {
        ...
        var fakePlugin = {
            startPlugin: function startPlugin(core) {
                oCoreObject = core;
                return core;
            }
        };
    ...
    var _getLabelForItem = function _getLabelForItem(oItem) {
        var aItems = _getAllLabels();
        return aItems && aItems[oItem.getId()] ? aItems[oItem.getId()] : null;
    };

    var _getUi5Id = function _getUi5Id(oItem) {
    ...
    var _getOwnerComponent = function _getOwnerComponent(oParent) {
        var sCurrentComponent = "";
        while (oParent && oParent.getParent) {
            if (oParent.getController && oParent.getController() && oParent.getController().getOwnerComponent && oParent.getController().getOwnerComponent()) {
                sCurrentComponent = oParent.getController().getOwnerComponent().getId();
    ...
    var _checkItem = function _checkItem(oItem, id) {
        var bFound = true;
        if (!oItem) {
            //e.g. parent level is not existing at all..
            return false;
        }
        if (id.metadata) {
    ...
    if (typeof id !== "string") {
        if ((0, _stringify2.default)(id) == (0, _stringify2.default)({})) {
            return [];
        }
    ...
    getUI5: (oDomNode, fn) => {
        ...
        var fnGetBindingContextInformation = function fnGetBindingContextInformation(oItem, sModel) {
            var oCtx = oItem.getBindingContext(sModel === "undefined" ? undefined : sModel);
            if (!oCtx) {
                return null;
            }
        ...
        var fnGetContextModels = function fnGetContextModels(oItem) {
            var oReturn = {};
        ...
        var fnGetElementInformation = function fnGetElementInformation(oItem, oDomNode, bFull) {
            var oReturn = {
                property: {},
                aggregation: [],
                association: {},
        ...
        var fnGetContexts = function fnGetContexts(oItem) {
            var oReturn = {};
